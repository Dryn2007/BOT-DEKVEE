...
class PrivateHelpDropdown(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if val == "Verifikasi & Kelas":
            ...
            embed.add_field(name="`!cekgate`", value="**Akses:** Administrator\n**Fungsi:** Diagnosa gerbang verifikasi — kenapa pesan **HALT** nggak muncul buat user baru. Cek intent members, permission bot & @everyone di room regis, Rules Screening, dan daftar member baru yang belum dapat instruksi. Read-only.", inline=False)
            embed.add_field(name="`!kirimhalt @user`", value="**Akses:** Administrator\n**Fungsi:** Paksa kirim ulang instruksi upload SKL ke satu member (room regis + DM).", inline=False)
